...
ap.add_argument(
    "-r", "--resizefactor", required=True, help="Downscaling factor, values 0.0 to 1.0"
)
...
ap.add_argument(
    ...
    "--kernelsize",
    required=False,
    help="Kernel size for Gaussian blur filtering",
)
ap.add_argument(
    "-sf",
    "--sobelfilter",
    required=False,
    action="store_true",
    help="If yes, the processed image will be filtered with sobel kernels",
)
...
try:
    compression_level = int(
        args["compression"]
    )  # Convert the compression level to an integer
    if compression_level < 0 or compression_level > 10:
        raise ValueError
except:
    print(
        "Invalid value for compression lvl. (Value has to be integer ranging from 0 to 10)"
    )
    exit()
...
cv.imwrite(
    "compressed_image.jpg", resized, [cv.IMWRITE_JPEG_QUALITY, compression_level]
)

if args["kernelsize"]:
    comp_img = cv.imread("compressed_image.jpg")
    filter_img = cv.GaussianBlur(
        comp_img, (kernel_size, kernel_size), cv.BORDER_DEFAULT
    )
else:
...
cv.line(
    filter_img,
    (0, filter_img.shape[0] // 2),
    (filter_img.shape[1], filter_img.shape[0] // 2),
    (0, 255, 0),
    2,
)
cv.putText(
    filter_img,
    "X (Global)",
    (filter_img.shape[1] - 60, filter_img.shape[0] // 2 - 10),
...
cv.line(
    filter_img,
    (filter_img.shape[1] // 2, 0),
    (filter_img.shape[1] // 2, filter_img.shape[0]),
    (0, 0, 255),
    2,
)
cv.putText(
