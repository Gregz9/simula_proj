...
def descrew(img, sorted_pts, real_width_cm, real_height_cm):
    ...
    dst = np.array([
        ...
        [0, dst_height_px - 1]], dtype='float32')
...
def create_graph(centres, img):
    ...
    for i, centre in enumerate(centres):
        pos_cm = (centre[0] / px_per_cm_x, centre[1] / px_per_cm_y)  # Calculate position in cm
    ...
    for i in range(len(centres)):
        for j in range(i+1, len(centres)):
            ...
            line_length_px = math.sqrt((centres[j][0] - centres[i][0])**2 + (centres[j][1] - centres[i][1])**2)
...
def draw_centre_and_lines(img, centres, color=(0, 0, 255), thickness=2):
    ...
    for i, centre in enumerate(centres):
        ...
        for other_centre in centres[i+1:]:
            ...
            line_length_px = math.sqrt((other_centre[0] - centre[0])**2 + (other_centre[1] - centre[1])**2)
            line_length_cm_x = line_length_px / px_per_cm_x
            line_length_cm_y = line_length_px / px_per_cm_y
            print(f'Length of line between {centre} and {other_centre} in x direction: {line_length_cm_x:.2f} cm')
            print(f'Length of line between {centre} and {other_centre} in y direction: {line_length_cm_y:.2f} cm')
    return img
